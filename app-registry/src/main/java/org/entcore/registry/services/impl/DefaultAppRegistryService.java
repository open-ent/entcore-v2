...
public class DefaultAppRegistryService implements AppRegistryService {
	...
	public void listApplications(String structureId, Handler<Either<String, JsonArray>> handler) {
		...
		String query =
				...
				"RETURN n.id as id, n.displayName as displayName, n.name as name, n.icon as icon, n.address as address, 'External' IN labels(n) as isExternal, n.levelsOfEducation as levelsOfEducation, n.appType as appType";
	...
	public void listStructuresByApplication(String appName, Integer page, Integer pageSize, Handler<Either<String, JsonObject>> handler) {
		...
		String dataQuery =
				"MATCH (app:Application {name: {appName}})-[:PROVIDE]->(:Action)<-[:AUTHORIZE]-(r:Role) " +
				"MATCH (r)<-[:AUTHORIZED]-(g:Group)-[:DEPENDS*1..2]->(s:Structure) " +
				"RETURN DISTINCT s.id as structureId, s.UAI as UAI, s.name as name " +
				"ORDER BY coalesce(s.UAI, '') ASC " +
	...
	public void listCasConnectors(final Handler<Either<String, JsonArray>> handler) {
		String query =
				...
				"RETURN app.inherits as inherits, app.casType as service, app.address as address, app.structureId as structureId, app.statCasType as statCasType, COLLECT(app.pattern) as patterns";
	...
	public void massAuthorization(JsonArray data, Integer transactionId, Boolean commit, Handler<Either<String, JsonObject>> handler) {
			...
			structure.getValue().entrySet().forEach(roleOrWidget -> {
				final String query =
						"MATCH (s:Structure {id: {structureId}}), " +
								...
								"CREATE UNIQUE g-[:AUTHORIZED]->r ";
				JsonObject params = new JsonObject().put("structureId", structure.getKey())
		...
		neo.executeTransaction(s.build(), transactionId, commit.booleanValue(), event -> {
