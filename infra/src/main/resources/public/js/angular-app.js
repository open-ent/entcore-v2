...
var template = {
  ...
  getCompletePath: function (view, isPortal) {
    ...
    if (
      skin.templateMapping[folder] &&
      skin.templateMapping[folder].indexOf(view) !== -1
    ) {
      path =
        "/assets/themes/" +
        skin.skin +
  ...
  loadPortalTemplates: function () {
    ...
    this.containers.portal["conversationUnread"] = this.getCompletePath(
      "conversation-unread",
      true
    );
  },
  ...
  contains: function (name, view) {
    return this.containers[name] === undefined
      ? view === undefined
      : this.containers[name].startsWith(this.viewPath + view + ".html");
  },
  ...
  watch: function (container, fn) {
    if (typeof fn !== "function") {
      throw TypeError(
        "template.watch(string, function) called with wrong parameters"
      );
    }
    if (!this.callbacks) {
      this.callbacks = {};
...
var module = angular
  .module("app", ["ngSanitize", "ngRoute"], function ($interpolateProvider) {
  ...
  .factory("route", function ($rootScope, $route, $routeParams) {
    ...
    $rootScope.$on(
      ...
      function ($currentRoute, $previousRoute) {
        if (
          typeof routes[$route.current.action] === "function" &&
  ...
  return {
    ...
    link: function (scope, element, attributes) {
      scope.$watch("field", function (newVal) {
        element.val(newVal);
        if (
          element[0].type === "textarea" &&
          element.hasClass("inline-editing")
        ) {
          setTimeout(function () {
            element.height(1);
...
module.directive("lightbox", function ($compile) {
  return {
    ...
    link: function (scope, element, attributes) {
      ...
      element
        .children(".lightbox")
        .find("> .background, > .content > .close-lightbox > i.close-2x")
        .on("click", function (e) {
        ...
        if (newVal) {
          ...
          scope.backup = {
            ...
            zIndex: _.map(parentElements, function (parent) {
              ...
              if (
                $(parent).attr("style") &&
                $(parent).attr("style").indexOf("z-index") !== -1
              ) {
...
module.directive("slider", function ($compile, $parse) {
  return {
    ...
    scope: true,
    template:
      '<div class="bar"></div><div class="filled"></div><div class="cursor"></div><legend class="min"></legend><legend class="max"></legend>',
    link: function (scope, element, attributes) {
      ...
      var applyValue = function (newVal) {
        var pos = parseInt(
          ((newVal - min) * element.children(".bar").width()) / (max - min)
        );
        cursor.css({
          left: pos + "px",
          position: "absolute",
      ...
      element
        .children("legend.min")
        .html(idiom.translate(attributes.minLegend));
      element
        .children("legend.max")
        .html(idiom.translate(attributes.maxLegend));

      element.children(".bar, .filled").on("click", function (e) {
        var newPos = e.clientX - element.children(".bar").offset().left;
        var newVal =
          (newPos * (max - min)) / element.children(".bar").width() + min;
      ...
      ui.extendElement.draggable(cursor, {
        ...
        mouseUp: function () {
          ...
          var newVal =
            (cursorPosition * (max - min)) / element.children(".bar").width() +
            min;
          ngModel.assign(scope, newVal);
          scope.$apply();
        },
  ...
  return {
    ...
    link: function (scope, element, attributes) {
      ...
      scope.$watch("ngModel", function (newVal) {
        if (
          (newVal && newVal._id) ||
          (newVal && scope.multiple && newVal.length)
        ) {
          scope.ngChange();
        }
  ...
  return {
    ...
    link: function (scope, element, attributes) {
      ...
      scope.$watch(
        ...
        function (newVal) {
          if (newVal) {
            ...
            scope.linker.search.application = _.find(
              scope.linker.apps,
              function (app) {
              ...
              Behaviours.loadBehaviours(app.prefix, function (behaviours) {
                ...
                if (n === 0) {
                  var currentApp = _.find(scope.linker.apps, function (app) {
                    return (
                      app.address.indexOf(appPrefix) !== -1 &&
                      app.icon.indexOf("/") === -1
                    );
                  });
      ...
      scope.linker.loadApplicationResources = function (cb) {
        ...
        scope.linker.addResource =
          Behaviours.applicationsBehaviours[prefix].create;
      };
      ...
      scope.linker.searchApplication = function () {
        ...
        Behaviours.loadBehaviours(
          scope.linker.params.appPrefix,
          function (appBehaviour) {
            scope.linker.resources = _.filter(
              ...
              function (resource) {
                return (
                  ...
                  (lang
                    ...
                    .indexOf(
                      lang.removeAccents(scope.linker.search.text).toLowerCase()
                    ) !== -1 ||
      ...
      scope.linker.saveLink = function () {
        ...
        if (scope.linker.params.link) {
          ...
          if (scope.linker.params.appPrefix) {
            ...
            if (
              scope.linker.params.appPrefix === appPrefix &&
              !scope.linker.externalLink
            ) {
    ...
    controller: function ($scope, $timeout) {
      var refreshCalendar = function () {
        ...
        $scope.createItem = function (day, timeslot) {
          $scope.newItem = {};
          var year =
            model.calendar.firstDay.year() || (day.date && day.date.year());
          ...
          $scope.newItem.beginning = moment()
            .utc()
            .year(year)
            .dayOfYear(day.index)
            .hour(timeslot.start);
          $scope.newItem.end = moment()
            .utc()
            .year(year)
            .dayOfYear(day.index)
            .hour(timeslot.end);
    ...
    link: function (scope, element, attributes) {
      ...
      attributes.$observe("createTemplate", function () {
        ...
        if (attributes.displayTemplate) {
          template.open(
            "schedule-display-template",
            attributes.displayTemplate
          );
        }
        if (attributes.displayMonthTemplate) {
          template.open(
            "schedule-display-month-template",
            attributes.displayMonthTemplate
          );
        }
        if (attributes.moreItemsTemplate) {
          template.open(
            "schedule-more-items-template",
            attributes.moreItemsTemplate
          );
        }
      });
...
module.directive("scheduleItem", function ($compile) {
  return {
    ...
    require: "^calendar",
    template:
      '<div class="schedule-item" resizable horizontal-resize-lock draggable>' +
    ...
    link: function (scope, element, attributes) {
      ...
      if (
        scope.item.beginning.dayOfYear() !== scope.item.end.dayOfYear() ||
        !scope.item.myRights.process ||
        moment().diff(moment(scope.item.end_date)) > 0 ||
        scope.item.parent_booking_id != null
      ) {
      ...
      var getTimeFromBoundaries = function () {
        ...
        var topPos =
          scheduleItemEl.position().top +
          calendar.dayHeight * calendar.startOfDay;
        var startTime = moment().utc();
        startTime.hour(Math.floor(topPos / calendar.dayHeight));
        startTime.minute(
          ((topPos % calendar.dayHeight) * 60) / calendar.dayHeight
        );

        var endTime = moment().utc();
        endTime.hour(
          Math.floor((topPos + scheduleItemEl.height()) / calendar.dayHeight)
        );
        endTime.minute(
          (((topPos + scheduleItemEl.height()) % calendar.dayHeight) * 60) /
            calendar.dayHeight
        );
      ...
      var placeItem = function () {
        ...
        while (collision) {
          ...
          scope.day.scheduleItems.forEach(function (scheduleItem) {
            ...
            if (
              scheduleItem.beginning < scope.item.end &&
              scheduleItem.end > scope.item.beginning
            ) {
        ...
        var beginningMinutesHeight =
          (scope.item.beginning.minutes() * calendar.dayHeight) / 60;
        var endMinutesHeight =
          (scope.item.end.minutes() * calendar.dayHeight) / 60;
        var top =
          (hours.startTime - calendar.startOfDay) * calendar.dayHeight +
          beginningMinutesHeight;
        scheduleItemEl.height(
          (hours.endTime - hours.startTime) * calendar.dayHeight -
            beginningMinutesHeight +
            endMinutesHeight +
            "px"
        );
        scheduleItemEl.css({
          top: top + "px",
          left:
            scope.item.calendarGutter * ((itemWidth * dayWidth) / 100) + "px",
        });
        ...
        if (top < 0) {
          ...
          container.height(
            element.children(".schedule-item").height() + top + 5
          );
        } else {
          container.css({
            top: 0 + "px",
...
module.directive("imageSelect", function ($compile) {
  return {
    ...
    template:
      '<div><img ng-src="[[ngModel]]?[[getThumbnails()]]" class="pick-file" draggable="false" ng-if="ngModel" style="cursor: pointer" />' +
    ...
    link: function (scope, element, attributes) {
      ...
      element.on("drop", function (e) {
        ...
        workspace.Document.prototype.upload(
          file,
          "file-upload-" + file.name + "-0",
          function (doc) {
    ...
    link: function ($scope, $element, $attributes) {
      var hideFunction = function (e) {
        ...
        if (
          !timepicker ||
          $element[0] === e.target ||
          $(".bootstrap-timepicker-widget").find(e.target).length !== 0
        ) {
          return;
        }
      ...
      loader.asyncLoad(
        "/" + infraPrefix + "/public/js/bootstrap-timepicker.js",
        function () {
          $element.timepicker({
            showMeridian: false,
            defaultTime: "current",
      ...
      $scope.$watch("ngModel", function (newVal) {
        ...
        if (
          $scope.ngLimit !== undefined &&
          !newVal.isSame($scope.ngLimit) &&
      ...
      scope.$watch("bindHtml", function (newVal) {
        ...
        htmlVal.find("*").each(function (index, item) {
          ...
          for (var i = attributes.length - 1; i >= 0; i--) {
            if (
              attributes[i].name.startsWith("on") ||
              attributes[i].value.includes("javascript:")
            ) {
        ...
        if (htmlVal.find("portal").length) {
          ...
          htmlVal = $(
            "<div>" + (portal.find("[bind-html]").html() || "") + "</div>"
          );
        }
...
module.directive("dropDown", function ($compile, $timeout) {
  return {
    ...
    template:
      ...
      '<li class="display-more" ng-show="limit < options.length" ng-click="increaseLimit()">' +
      lang.translate("seemore") +
      "</li>" +
      "</ul>" +
      "</div>" +
      "</div>",
    link: function (scope, element, attributes) {
      ...
      if (dropDownContent) {
        if (scope.listId) dropDownContent.attr("id", scope.listId);
      }
    ...
    link: function (scope, element, attributes) {
      ...
      scope.$watch("search", function (newVal) {
        ...
        scope.match = _.filter(scope.options(), function (option) {
          ...
          return (
            _.find(words, function (word) {
              var formattedOption = lang
                .removeAccents(option.toString())
                .toLowerCase();
...
var ckeEditorFixedPositionning = function () {
  ...
  for (var instance in CKEDITOR.instances) {
    ...
    $("head")
      .find("[ckestyle=" + CKEDITOR.instances[instance].name + "]")
      .remove();
    ...
    toolbox.width(
      editableElement.width() +
        2 +
        parseInt(editableElement.css("padding") || 4) * 2
    );
    $('<style ckestyle="' + CKEDITOR.instances[instance].name + '"></style>')
      .text(
        "#cke_" +
          CKEDITOR.instances[instance].name +
          "{" +
          "top:" +
          (editableElement.offset().top - toolbox.height()) +
          "px !important;" +
          "left:" +
          editableElement.offset().left +
          "px !important;" +
...
function createCKEditorInstance(editor, scope, $compile, attributes) {
  ...
  editor.on("keyup", function () {
    editor
      .find("[bind-html], [ng-transclude]")
      .removeAttr("bind-html")
...
module.directive("textEditor", function ($compile) {
  return {
    ...
    template:
      '<div contenteditable="true" style="width: 100%;" class="contextual-editor"></div>' +
    ...
    compile: function () {
      ...
      if (window.CKEDITOR === undefined) {
        ...
        CKEDITOR.plugins.basePath =
          "/" + infraPrefix + "/public/ckeditor/plugins/";
      }
      return function (scope, element, attributes) {
        if (!scope.display) {
          scope.display = {};
        ...
        editor.on("click", function () {
          if (
            parentElement.data("resizing") ||
            parentElement.data("dragging")
          ) {
            return;
          }
          editor.focus();
        ...
        $("body").on(
          "click",
          "#cke_" + instance.name + " .cke_button__linker",
          function () {
        ...
        $("body").on(
          "click",
          "#cke_" + instance.name + " .cke_button__html",
          function () {
            scope.editHtml = true;
...
module.directive("htmlEditor", function ($compile, $parse) {
  return {
    ...
    template:
      '<div class="twelve cell block-editor">' +
    ...
    compile: function (element, attributes, transclude) {
      ...
      return function (scope, element, attributes) {
          ...
          function (newValue) {
            if (contextEditor.getData() !== newValue) {
              ...
              editor.find("audio").each(function (index, item) {
                var parent = $(item).parent();
                $(item).attr("src", item.src).detach().appendTo(parent);
              });
            }
          }
        );
        ...
        $("body").on(
          "click",
          "#cke_" + contextEditor.name + " .cke_button__upload",
          function () {
        ...
        $("body").on(
          "click",
          "#cke_" + contextEditor.name + " .cke_button__audio",
          function () {
            scope.selectFiles = true;
            scope.format = "audio";
            scope.$apply("selectFiles");
        ...
        $("body").on(
          "click",
          "#cke_" + contextEditor.name + " .cke_button__linker",
          function () {
        ...
        $("body").on(
          "click",
          "#cke_" + contextEditor.name + " .cke_button__video",
          function () {
        ...
        $("body").on(
          "click",
          "#cke_" + contextEditor.name + " .cke_button__html",
          function () {
            scope.editHtml = true;
            scope.selected.html = editor.html();
            scope.$apply("editHtml");
        ...
        scope.addContent = function () {
          scope.selected.files.forEach(function (file) {
            ...
            if (scope.format === "img") {
              ...
              if (attributes.resizeImages) image.setAttribute("resizable", "");
    ...
    link: function ($scope, $element, $attributes) {
      var addImage = function () {
        ...
        if ($("body").find("admin-portal").length) {
          var loadingIllustrationPath =
            "/public/" + skin.skin + "/img/icons/anim_loading_small.gif";
        } else {
          var loadingIllustrationPath =
            "/assets/themes/" + skin.skin + "/img/icons/anim_loading_small.gif";
        }
      ...
      if ($attributes.onlyLoadingIcon === undefined) {
        http().bind("request-ended." + $attributes.request, function (e) {
          var loadingDonePath =
            "/assets/themes/" + skin.skin + "/img/icons/checkbox-checked.png";
    ...
    link: function ($scope, $element, $attributes) {
      $attributes.$observe("loadingPanel", function (val) {
        http().bind(
          ...
          function (e) {
            var loadingIllustrationPath =
              "/assets/themes/" + skin.skin + "/img/illustrations/loading.gif";
  ...
  return {
    ...
    link: function (scope, element, attributes) {
      ...
      if (attributes.tooltipRestrictSelector) {
        restrictToElement = tgtElement.parents(
      ...
      tgtElement.on("mouseenter", function () {
        if (attributes.tooltipTemplate) {
          ...
          tip = $("<div />")
            .addClass("tooltip")
            .html(
              $compile("<div ng-include='\"" + tplPath + "\"'></div>")(scope)
            )
            .appendTo("body");
          scope.$apply();
          ...
          if (restrictToElement) {
            if (left < restrictToElement.offset().left) {
              left = parseInt(
                tgtElement.offset().left + tgtElement.width() + 5
              );
            }

            // now, is it too far right ?
            if (
              left + tgtElement.width() >
            ...
            .html(
              $compile(
                '<div class="arrow"></div><div class="content">' +
                  lang.translate(attributes.tooltip) +
                  "</div> "
              )(scope)
            )
            .appendTo("body");
          ...
          var left = parseInt(
            tgtElement.offset().left + tgtElement.width() / 2 - tip.width() / 2
          );
          if (top < 5) {
            top = 5;
          }
  ...
  return {
    ...
    link: function (scope, element, attributes) {
      ...
      scrollElement.scroll(function () {
        var scrollHeight =
          scrollElement[0].scrollY || scrollElement[0].scrollTop;
...
module.directive("gridCell", function ($compile) {
  return {
    ...
    template:
      '<div class="media-wrapper"><div class="media-container" ng-class="className" ng-transclude></div></div>',
    transclude: true,
    link: function (scope, element, attributes) {
      var cellSizes = [
        "zero",
    ...
    link: function (scope, element, attributes) {
      ...
      element.on("mouseover", function (e) {
        element.on("mousemove", function (e) {
          ...
          lock.vertical =
            element.find("grid-cell, sniplet, [vertical-lock]").length > 0;
          ...
          resizeLimits = {
            horizontal:
              element.offset().left + element.width() + 5 > mouse.x &&
              mouse.x > element.offset().left + element.width() - 15,
            vertical:
              element.offset().top +
      ...
      element.on("mousedown.resize", function (e) {
        if (
          ...
          (!resizeLimits.horizontal && !resizeLimits.vertical)
        ) {
          return;
        }
        var mouse = {
        ...
        $(window).on("mousemove.resize", function (e) {
          ...
          var resize = function () {
            ...
            if (resizeLimits.horizontal) {
              ...
              if (
                element.offset().left + distance >
                parentData.pos.left + parentData.size.width
              ) {
                distance =
                  parentData.pos.left +
                  parentData.size.width -
                  element.offset().left -
                  2;
              }
              newWidth = distance;
              ...
              var neighbour = findResizableNeighbour(
                element,
                distance - element.width()
              );
  ...
  return {
    ...
    link: function (scope, element, attributes) {
      ...
      element.on("mousedown", function (e) {
        ...
        if (
          element.data("lock") === true ||
          element.find("editor").data("lock") === true ||
          parent.first().hasClass("blur-grid")
        ) {
          return;
        }
        ...
        $(window).on("mouseup.drag", function (e) {
          ...
          if (element.data("dragging") === true) {
            ...
            parent.find(".grid-row").each(function (index, item) {
              if (
                elementPos.top + elementPos.height / 2 > $(item).offset().top &&
                elementPos.top + elementPos.height / 2 <
                  $(item).offset().top + $(item).prev().height()
              ) {
                if (!rowFull($(item))) {
                  setTimeout(function () {
        ...
        var moveElement = function (elementDistance) {
          ...
          parent.find(".grid-row").each(function (index, item) {
            if (
              newOffset.top + elementPos.height / 2 > $(item).offset().top &&
              newOffset.top + elementPos.height / 2 <
                $(item).offset().top + $(item).prev().height()
            ) {
              row = $(item);
              return false;
...
module.directive("sniplet", function ($parse, $timeout) {
  return {
    ...
    controller: function ($scope, $timeout) {
      $timeout(function () {
        Behaviours.loadBehaviours($scope.application, function (behaviours) {
          var snipletControllerExpansion =
            behaviours.sniplets[$scope.template].controller;
    ...
    template:
      "<div ng-include=\"'/' + application + '/public/template/behaviours/sniplet-' + template + '.html'\"></div>",
    link: function (scope, element, attributes) {
...
module.directive("snipletSource", function ($parse, $timeout) {
  return {
    ...
    scope: true,
    template:
      "<div ng-include=\"'/' + application + '/public/template/behaviours/sniplet-source-' + template + '.html'\"></div>",
    controller: function ($scope, $timeout) {
  ...
  return {
    ...
    link: function (scope, element) {
      ...
      var toTop = function () {
        ...
        element
          .parents(".drawing-zone")
          .find("article[draggable]")
  ...
  function ($rootScope) {
    return {
      ...
      link: function ($scope, $element, $attributes) {
        ...
        if (
          !($scope.resources instanceof Array) &&
          !$scope.resources.myRights
        ) {
          throw new TypeError(
            "Resources in share panel must be instance of Array or implement Rights interface"
          );
        }
        ...
        http()
          .get("/" + infraPrefix + "/public/json/sharing-rights.json")
        ...
        function actionToRights(item, action) {
          var actions = [];
          _.where($scope.actions, { displayName: action.displayName }).forEach(
            function (item) {
          ...
          function different(type) {
            for (var element in model1[type].checked) {
              ...
              model1[type].checked[element].forEach(function (right) {
                result =
                  result || model2[type].checked[element].indexOf(right) === -1;
              });
            }
        ...
        var feedData = function () {
          ...
          $scope.resources.forEach(function (resource) {
            var id = resource._id;
            http()
              .get("/" + currentApp + "/share/json/" + id + "?search=")
              .done(function (data) {
                if (initModel) {
                ...
                var editResource =
                  $scope.editResources[$scope.editResources.length - 1];
                ...
                if (!initModel) {
                  if (
                    differentRights(editResource, $scope.sharingModel) ||
                    differentRights($scope.sharingModel, editResource)
                  ) {
        ...
        $scope.findUserOrGroup = function () {
          ...
          if (
            !usersCache[startSearch] &&
            !(usersCache[startSearch] && usersCache[startSearch].loading)
          ) {
            usersCache[startSearch] = { loading: true };
            var id = $scope.resources[0]._id;
            var path =
              "/" + currentApp + "/share/json/" + id + "?search=" + startSearch;
            if (!startSearch) {
          ...
          $scope.found = _.union(
            _.filter($scope.sharingModel.groups.visibles, function (group) {
              ...
              return (
                ...
                _.find($scope.sharingModel.edited, function (i) {
            ...
            _.filter($scope.sharingModel.users.visibles, function (user) {
              var testName = idiom
                .removeAccents(user.lastName + " " + user.firstName)
                .toLowerCase();
              var testNameReversed = idiom
                .removeAccents(user.firstName + " " + user.lastName)
                .toLowerCase();
              return (
                (testName.indexOf(searchTerm) !== -1 ||
        ...
        $scope.remove = function (element) {
          ...
          $scope.sharingModel.edited = _.reject(
            $scope.sharingModel.edited,
            function (item) {
        ...
        function applyRights(element, action, cb) {
          ...
          if (!element.actions[action.displayName]) {
            ...
            _.filter($scope.actions, function (item) {
              return (
                _.find(item.requires, function (dependency) {
                  return (
                    action.displayName.split(".")[1].indexOf(dependency) !== -1
                  );
                }) !== undefined
              );
            ...
            action.requires.forEach(function (required) {
              var action = _.find($scope.actions, function (action) {
                return (
                  action.displayName.split(".")[1].indexOf(required) !== -1
                );
              });
              if (action) {
                element.actions[action.displayName] = true;
                data.actions = data.actions.concat(
                  actionToRights(element, action)
                );
              }
            });
          }
          ...
          $scope.resources.forEach(function (resource) {
            http()
              .put(
                "/" + currentApp + "/share/" + setPath + "/" + resource._id,
                http().serialize(data)
              )
              .done(function () {
                if (setPath === "remove") {
    ...
    compile: function (element, attributes, transclude) {
      ...
      return function (scope, element, attributes) {
        scope.updateElementsOrder = function (el) {
          ...
          if (
            el.offset().top >
            sortables.last().offset().top + sortables.last().height()
          ) {
      ...
      ui.extendElement.draggable(element, {
        ...
        tick: function () {
          ...
          sortables.each(function (index, sortable) {
            ...
            var sortableTopDistance =
              $(sortable).offset().top -
              parseInt($(sortable).css("margin-top"));
            if (
              element.offset().top + element.height() / 2 >
                sortableTopDistance &&
              element.offset().top + element.height() / 2 <
                sortableTopDistance + $(sortable).height()
            ) {
              $(sortable).css({
            ...
            //first widget case
            if (
              element.offset().top + element.height() / 2 - 2 <
                sortableTopDistance &&
              index === 0
            ) {
              $(sortable).css({
    ...
    link: function (scope, element, attributes) {
      element.on("index-changed", ".widget-container", function (e) {
        ...
        element.find(".widget-container").each(function (index, widget) {
          ...
          if (
            $(e.target).offset().top + $(e.target).height() / 2 >
              $(widget).offset().top &&
            $(e.target).offset().top + $(e.target).height() / 2 <
              $(widget).offset().top + $(widget).height()
          ) {
          ...
          if (
            $(e.target).offset().top >
              $(widget).offset().top + $(widget).height() &&
            index === element.find(".widget-container").length - 1
          ) {
            widgetObj.setIndex(index);
            scope.$apply("widgets");
          }
          //first widget case
          if (
            $(e.target).offset().top + $(e.target).height() >
              $(widget).offset().top &&
            index === 0
          ) {
            widgetObj.setIndex(index);
...
module.directive("datePicker", function ($compile) {
  return {
    ...
    restrict: "E",
    template:
      '<input ng-transclude type="text" data-date-format="dd/mm/yyyy"  />',
    link: function (scope, element, attributes) {
      scope.$watch("ngModel", function (newVal) {
        ...
        if (element.datepicker)
          element.datepicker(
            "setValue",
            moment(scope.ngModel).format("DD/MM/YYYY")
          );
      });
      ...
      loader.asyncLoad(
        "/" + infraPrefix + "/public/js/bootstrap-datepicker.js",
        function () {
          element
            .datepicker({
              dates: {
...
module.directive("datePickerIcon", function ($compile) {
  return {
    ...
    restrict: "E",
    template:
      '<div class="date-picker-icon"> <input type="text" class="hiddendatepickerform" style="visibility: hidden; width: 0px; height: 0px; float: inherit" data-date-format="dd/mm/yyyy"/> <a ng-click="openDatepicker()"><i class="calendar"/></a> </div>',
    link: function ($scope, $element, $attributes) {
      loader.asyncLoad(
        "/" + infraPrefix + "/public/js/bootstrap-datepicker.js",
        function () {
      ...
      ui.extendElement.draggable(element, {
        ...
        tick: function (e, mouse) {
          ...
          $("[drop-item]").each(function (index, el) {
            if (
              $(el).offset().left < mouse.x &&
              $(el).offset().left + $(el).width() > mouse.x &&
              $(el).offset().top - (window.scrollY || window.pageYOffset) <
                mouse.y &&
              $(el).offset().top +
                $(el).height() -
...
module.directive("lightAttachments", function ($parse) {
  return {
    scope: true,
    restrict: "E",
    templateUrl: "/" + infraPrefix + "/public/template/attachments.html",
    ...
    link: function (scope, element, attributes) {
      ...
      scope.linker.createResource = function () {
        ...
        Behaviours.loadBehaviours(prefix, function (appBehaviour) {
          var backup = appBehaviour.loadResources;
          appBehaviour.loadResources = function (cb) {
            cb && cb();
          };
          appBehaviour.create(
            scope.linker.resource,
            function (rr, newResource) {
              appBehaviour.loadResources = backup;
              if (!(scope.ngModel(scope) instanceof Array)) {
                scope.ngModel.assign(scope, []);
              }
              newResource.title = newResource.name;
              scope.attachments.display.pickFile = false;
              scope.ngModel(scope).push(newResource);
              scope.$apply();
            }
          );
        });
    ...
    compile: function (element, attributes, transclude) {
      return function (scope, element, attributes) {
        ...
        steps.each(function (index, item) {
          nav
            .children("ul")
            .append("<li>" + $(item).find("h1").html() + "</li>");
        });
        ...
        scope.nextCondition = function () {
          ...
          if (typeof stepScope.nextCondition() === "undefined") return true;
          return stepScope.nextCondition();
        };

        scope.nextStep = function () {
          if (!scope.nextCondition()) return;
        ...
        scope.checkFinishCondition = function () {
          if (typeof scope.finishCondition() === "undefined") return true;
          return scope.finishCondition();
        };

        scope.finish = function () {
          if (!scope.checkFinishCondition()) return;
  ...
  return {
    ...
    link: function (scope, element, attributes) {
      ...
      var imageHeight = function () {
        element
          .find(".current img")
          .height(element.find(".current .image-container").height());
  ...
  return {
    ...
    link: function (_scope, element, attributes) {
      if (navigator.pdfViewerEnabled) {
        ...
        span.innerText = lang.translate("embed.video.incompatible");
        element.append(span);
      }
    },
  };
    ...
    link: function (scope, element, attributes) {
      ...
      scope.fullscreen = function (allow) {
        ...
        if (allow) {
          ...
          renderElement = element.find(".render");
          renderParent = renderElement.parent();

          renderElement.addClass("fullscreen").appendTo(container);
...
module.directive("explorer", function () {
  return {
    ...
    link: function (scope, element, attributes) {
      ...
      $("body").on("click", function (e) {
        if (
          $(e.target).parents("explorer, .toggle, .lightbox").length === 0 &&
  ...
  return {
    ...
    link: function (scope, element, attributes) {
      ...
      scope.updatePreview = function () {
        ...
        label: for (
          var $index1 = 0;
          $index1 < scope.display.provider.url.length;
          $index1++
        ) {
...
(function () {
  ...
  function _getPreference(cb) {
    ...
    http()
      .get("/userbook/preference/authenticatedConnectorsAccessed")
      .done(function (data) {
        try {
          if (data.preference) {
          ...
          console.log(
            "Error parsing authenticatedConnectorsAccessed preferences"
          );
          _CACHE = {};
          return cb(_CACHE);
        }
  ...
  module.directive("connectorLightboxTrigger", [
    "$timeout",
    "$filter",
  ...
  module.directive("connectorLightbox", [
    "$timeout",
    "$filter",
    function ($timeout, $filter) {
      return {
        ...
        link: function (scope, element, attributes) {
          ...
          function isAuthenticatedConnector(app) {
            return (
              !!app.casType ||
              (app.scope && app.scope.length > 0 && !!app.scope[0])
            );
          }

          function isAuthenticatedConnectorFirstAccess(app) {
            return (
              !scope.authenticatedConnectorsAccessed ||
              (scope.authenticatedConnectorsAccessed &&
                scope.authenticatedConnectorsAccessed.indexOf(app.name) == -1)
            );
          }
          //public functions
            ...
            if (_app.isExternal && window.xiti && window.xiti.click) {
              if (target == "_self") {
                ...
                /* If target is a new page, Firefox would block the connector as a popup, hence we don't
                            ...
                            */
                window.xiti.click(_app.name, _app.element[0]);
                window.open(address, target);
              }
            } else {
              window.open(address, target);
          ...
          scope.onConfirm = function () {
            ...
            if (target != "_self") {
              http().putJson(
                "/userbook/preference/authenticatedConnectorsAccessed",
                scope.authenticatedConnectorsAccessed
              );
          ...
          function openAppWithCheck(app) {
            ...
            if (
              isAuthenticatedConnector(app) &&
              isAuthenticatedConnectorFirstAccess(app)
            ) {
              if (_MUTEX == true) {
                return;
              }
    ...
    link: function (scope, element, attributes) {
      if (
        ...
        !model.me.hasWorkflow(
          "org.entcore.portal.controllers.PortalController|quickstart"
        )
      ) {
        return;
      }
      ...
      if (typeof pulsarInfos !== "object" || pulsarInfos.index === undefined) {
        console.error(
          "Invalid pulsar object. Should look like pulsar=\"{ index: 0, i18n: 'my.key', position: 'top bottom'}\""
        );
      }
      ...
      var paintPulsar = function () {
        ...
        pulsarButton = $(
          '<div class="pulsar-button"><div class="pulse"></div><div class="pulse2"></div><div class="pulse-spot"></div></div>'
        ).appendTo("body");
        ...
        var placePulsar = function () {
          ...
          var xPositions = {
            ...
            center:
              element.offset().left + element.width() / 2 - pulsarSize / 2,
          };
          ...
          var yPositions = {
            ...
            center:
              element.offset().top + element.height() / 2 - pulsarSize / 2,
          };
          ...
          if (pulsarButton.css("display") !== "none") {
            pulsarButton.offset({
              left: parseInt(xPositions[xPosition] + deltaX),
              top: parseInt(yPositions[yPosition] + deltaY),
            });
          }
          ...
          if (pulsarElement && pulsarElement.find(".arrow").length) {
            var left =
              ...
              pulsarElement.children(".content").width() -
              pulsarSize / 2;
            ...
            if (yPosition === "top" && xPosition === "center") {
              top =
                ...
                pulsarElement.children(".content").height() -
                pulsarSize / 2;
            }
            if (yPosition === "center") {
              top =
                yPositions[yPosition] -
                pulsarElement.children(".content").height() / 2;
            ...
            if (xPosition === "center") {
              left =
                xPositions[xPosition] -
                pulsarElement.children(".content").width() / 2 -
                2;
            }
            ...
            if (yPosition === "bottom") {
              if (xPosition === "center") {
                top =
                  yPositions[yPosition] + pulsarLayerMarge + pulsarMarge * 2;
              } else {
            ...
            var newLeft =
              $(window).width() - (pulsarElement.width() + pulsarElementMarge);
            var newBottom =
              $(window).height() -
              (pulsarElement.height() + pulsarElementMarge);
            ...
            if (maxX > $(window).width()) {
              left =
                $(window).width() -
                (pulsarElement.width() + pulsarElementMarge);
              if (xPosition === "center") {
                if (firstCycle) {
            ...
            if (left <= pulsarElementMarge) {
              ...
              if (xPosition === "center") {
                if (firstCycle) {
                  arrow.css({
                    left: gapRight + pulsarLayerMarge,
                    right: "auto",
                  });
                  firstCycle = false;
                }
        ...
        var placeLayers = function () {
          ...
          $('<div class="pulsar-layer"></div>')
            ...
            .offset({
              top: pulsarHighlight.offset().top - pulsarLayerMarge,
              left: pulsarHighlight.offset().left - pulsarLayerMarge,
            })
            .hide()
            .appendTo("body")
            .fadeIn("slow");
        ...
        pulsarButton.on("click", function () {
          ...
          http()
            .get("/infra/public/template/pulsar.html")
            .done(function (html) {
      ...
      function undraw() {
        ...
        pulsarButton.fadeOut("slow", function () {
          pulsarButton.remove();
        });
        pulsarElement.fadeOut("slow", function () {
      ...
      scope.closePulsar = function () {
        ...
        pulsarElement.fadeOut(0, function () {
          pulsarElement.remove();
        });
        pulsarButton.removeClass("hidden");
        $(".pulsar-layer").fadeOut(0, function () {
          $(".pulsar-layer").remove();
        });
        $("body").off("scroll touchmove mousewheel");
        $("body").css("pointer-events", "");
      ...
      scope.isLastStep = function () {
        return (
          _.find(scope.pulsarInfos.steps, function (step) {
      ...
      scope.next = function () {
        ...
        if (
          _.findWhere(scope.pulsarInfos.steps, { index: index }) === undefined
        ) {
          if (
            _.find(scope.pulsarInfos.steps, function (item) {
    ...
    http()
      ...
      .done(function (result) {
        ...
        if (
          ...
          $scope.avatar =
            "/assets/themes/" + skin.skin + "/img/illustrations/no-avatar.svg";
        }
...
function WidgetModel() {
  ...
  model.collection(Widget, {
    ...
    sync: function () {
      ...
      http()
        .get("/userbook/preference/widgets")
        .done(
          function (pref) {
            if (!pref.preference) {
              this.preferences = {};
            ...
            data = data.map(function (widget, i) {
              ...
              widget.hide = widget.mandatory
                ? false
                : that.preferences[widget.name].hide;
              return widget;
            });
...
function Widgets($scope, model, lang, date) {
  ...
  $scope.allowedWidget = function (widget) {
    return (
      (!$scope.list || $scope.list.indexOf(widget.name) !== -1) && !widget.hide
    );
  };
...
var workspace = {
  ...
  Document: function (data) {
    ...
    this.protectedDuplicate = function (callback) {
      Behaviours.applicationsBehaviours.workspace.protectedDuplicate(
        this,
        function (data) {
    ...
    this.publicDuplicate = function (callback) {
      Behaviours.applicationsBehaviours.workspace.publicDuplicate(
        this,
        function (data) {
          callback(new workspace.Document(data));
        }
  ...
  Folder: function (data) {
    ...
    this.collection(workspace.Folder, {
      sync: function () {
        this.load(
          _.filter(
            model.mediaLibrary.myDocuments.folders.list,
            function (folder) {
...
workspace.Document.prototype.upload = function (
  file,
  requestName,
  callback,
  visibility
  ...
  http()
    .postFile(
      "/workspace/document?" +
        visibility +
        "=true&application=media-library&quality=0.7&" +
...
function MediaLibrary($scope) {
  ...
  $scope.openFolder = function (folder) {
    if (
      ...
      folder.folder.indexOf($scope.openedFolder.folder + "_") === -1
    ) {
      $scope.openedFolder.closeFolder();
    }
  ...
  $scope.$watch("visibility", function (newVal) {
    ...
    model.mediaLibrary.on(
      "myDocuments.sync, sharedDocuments.sync, appDocuments.sync, publicDocuments.sync",
      function () {
        $scope.documents = filteredDocuments(
          model.mediaLibrary[$scope.display.listFrom]
        );
        if (model.mediaLibrary[$scope.display.listFrom].folders) {
          $scope.folders = model.mediaLibrary[
            $scope.display.listFrom
          ].folders.filter(function (folder) {
            return (
              lang
                .removeAccents(folder.name.toLowerCase())
                .indexOf(
                  lang.removeAccents($scope.display.search.toLowerCase())
                ) !== -1
            );
          });
          $scope.$apply("folders");
    ...
    $scope.$watch("fileFormat", function (newVal) {
      ...
      if (
        model.mediaLibrary[$scope.display.listFrom].documents.length() === 0
      ) {
        model.mediaLibrary[$scope.display.listFrom].sync();
      } else {
  ...
  function filteredDocuments(source) {
    return source.documents.filter(function (doc) {
      return (
        ...
        lang
          .removeAccents(doc.metadata.filename.toLowerCase())
          .indexOf(lang.removeAccents($scope.display.search.toLowerCase())) !==
          -1
      );
    });
  }
  ...
  $scope.selectDocument = function (document) {
    if (
      ($scope.folder === model.mediaLibrary.appDocuments &&
        $scope.visibility === "protected") ||
      ($scope.folder === model.mediaLibrary.publicDocuments &&
  ...
  $scope.importFiles = function () {
    ...
    for (var i = 0; i < $scope.upload.files.length; i++) {
      ...
      workspace.Document.prototype.upload(
        ...
        "file-upload-" + $scope.upload.files[i].name + "-" + i,
        function () {
          waitNumber--;
          model.mediaLibrary.appDocuments.documents.sync();
          if (!waitNumber) {
      ...
      link: function (scope, element, attributes) {
        ...
        if (model.me.needRevalidateTerms && !model.me.deletePending) {
          var _readyCguLightbox = function () {
            ...
            scope.link = idiom
              .translate("cgu.revalidate.accept")
              .replace("[[linkStart]]", "<a href='" + scope.linkUrl + "'>")
              .replace("[[linkEnd]]", "</a>");
